...
from NetClass import CrossNet
net=CrossNet()
#net=nn.DataParallel(net)


optimizer = optim.SGD(net.parameters(), lr=0.11, momentum = 0.1)
...
print(net(dataLearn[1][20].unsqueeze(0)))
...
for i in range(min(net.conv1.weight.shape[0],8)):
    imshow(w2[i].squeeze(0),axs[i//4,i%4])
